...
%% Abstract section.
\abstract{
In many cases, protein reactions with other small molecules (ligands) occur at a deeply buried active site.
When studying these types of reactions it is crucial for biochemists to examine trajectories of ligand motion.
These trajectories are predicted with in-silico methods that produce large ensembles of possible trajectories.
In this paper, we propose a novel approach to the interactive visual exploration and analysis of large sets of trajectories enabling domain experts to assess the relevance of individual trajectories based on their properties.
The proposed solution is composed of multiple linked 2D and 3D views, enabling the interactive exploration and filtering of trajectories in an informed way.
In the workflow, we focus on the practical aspects of the interactive visual analysis specific to ligand trajectories.
We adopt and adapt small multiples to resolve an overly large number of trajectories into smaller chunks that are easier to analyze.
We describe how drill-down techniques can be used to create and store selections of trajectories with desired properties, enabling the comparison of multiple datasets.
In appropriately crafted 2D and 3D views, biochemists can either observe individual trajectories or choose to aggregate the information into a functional boxplot or density visualization.
Our solution is based on a tight collaboration with domain experts in order to address their needs as much as possible.
The usefulness of this novel approach is demonstrated by two case studies, conducted by the collaborating protein engineers.


%Exploration of protein reactions with other molecules plays a crucial role in biochemical research, including protein engineering and drug design.
%In many cases, a small molecule (ligand) reacts with the protein at a deeply buried active site. 
%As the ligand has to get to this site from the outer environment, it is crucial to study trajectories of ligand motion.
%There are several in-silico methods to calculate potential trajectories.
...
%These trajectories have to be explored by biochemists who are trying to assess their relevance and gain insight into the protein function.
...
%In this paper, we introduce a novel tool for the interactive visual exploration and analysis of ligand trajectories and their various properties.
%The tool has been designed in tight collaboration with domain experts in order to address their needs as much as possible.
%It is composed of several linked 2D and 3D views, enabling the interactive exploration and filtering of trajectories.
%Small multiples are used for communicating the properties of the trajectories.
...
\begin{itemize}
	\item General molecular vis:
		\begin{itemize}
			\item Visual Analysis of Biomolecular Cavities: State of the Art
			\item Visualization of biomolecular structures: State of the art revisited.
		\end{itemize}
  	\item Trajectories visualization
  		\begin{itemize}
  			\item Interactive Visual Analysis of Families of Function Graphs
  			\item Curve Boxplot: Generalization of Boxplot for Ensembles of Curves
  			\item Streamline Variability Plots for Characterizing the Uncertainty in Vector Field Ensembles
  			\item {\color{red} simplifikace ve 3D, maybe not related} SimpliFly: A Methodology for Simplification and Thematic Enhancement of Trajectories
  			\item Generalized box-plot for root growth ensembles
  		\end{itemize}
  	\item relevant for the modulate 3D visualization
  		\begin{itemize}
  			\item Game On, Science - How Video Game Technology May Help Biologists Tackle Visualization Challenges
  		\end{itemize}
  	\item relevant for density visualization
  		\begin{itemize}
  			\item Spatial Generalization and Aggregation of Massive Movement Data
  			\item Space–time density of trajectories: exploring spatio-temporal patterns in movement data
  		\end{itemize}
  	\item relevant for selection by density
  		\begin{itemize}
  			\item Visualizing Dynamical Systems near Critical Points
  			\item Strategies for Interactive Exploration of 3D Flow Using Evenly-Spaced Illuminated Streamlines
  		\end{itemize}
  	\item Trajectories in molecular context
  		\begin{itemize}
  			\item Watergate: Visual Exploration of Water Trajectories in Protein Dynamics
			\item Interactive Exploration of Ligand Transportation through Protein Tunnels
  			\item MolPathFinder: Interactive Multi-Dimensional Path Filtering of Molecular Dynamics Simulation Data
  			\item {\color{red}not completely relevant} Methodologies for the Analysis of Instantaneous Lipid Diffusion in MD Simulations of Large Membrane Systems
  			\item {\color{red}maybe not relevant} Heterogeneity-based Guidance for Exploring Multiscale Data in Systems Biology
			\item {\color{red}water trajectories - 3D} Visual Abstractions of Solvent Pathlines near Protein Cavities
			\item {\color{red}water trajectories - 3D - domain paper} Tracking the Flow of Water through Photosystem II Using Molecular
			\item {\color{red}water trajectories - 3D - domain paper - visualization by pymol} AQUA-DUCT: a ligands tracking tool Dynamics and Streamline Tracing
  		\end{itemize}
  \item Missing?
\end{itemize}
