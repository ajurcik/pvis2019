...
These trajectories are currently predicted with in-silico methods that usually produce large ensembles of possible trajectories. 
In this paper, we propose a novel approach to visual exploration and analysis of these large datasets allowing domain experts to assess the relevance of individual trajectories based on their properties.
...
In the workflow we describe we focus on practical aspects of the interactive visual analysis specific to the ligand trajectories data.
We adopted the well-proven approach of small multiples which allows us to break down the large dataset into smaller chunks that are easier to operate.
Here we describe how the drill down technique can be used to create and store selections of trajectories with desired properties which allows us to compare multiple datasets.
In both 2D and 3D views, biochemists can either observer the individual trajectories or choose to aggregate the information into a functional boxplot or a density visualization.
The tool was developed in tight collaboration with domain experts in order to address their needs as much as possible.
